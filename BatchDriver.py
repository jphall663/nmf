...
Ported to Python 3 (minimal changes)
"""
...
def main():
    ...
    nthread = 1
    working_dir = 'results'
    raw_file = 'data/mlhra_corpus.txt'
    stop_dir = 'stoplist'
    threshold = 3 # lower means more resolution in topics, but also overfitting
    custom_replacements = 'replacements/rai_replacements.txt'
    make_replacements = False

    n_features = 250  # k, topics or no. of features in the NMF
    ...
    if make_replacements:
        ...
        add_dict = load_dict('replacements' + os.sep + 'add.txt')
        print('Add dictionary contains %i terms.' % (len(add_dict)))

        change_dict = load_dict('replacements' + os.sep + 'change.txt')
        print('Change dictionary contains %i terms.' % (len(change_dict)))

        del_dict = load_dict('replacements' + os.sep + 'delete.txt')
        ...
        with open(working_dir + os.sep + 'replacements.txt', 'w') as repl_out:
